...
class Time:
    ...
    def format(self, format_string):
        if "%H" in format_string:
            format_string = format_string.replace("%H", str(self.hour).zfill(2))
        if "%M" in format_string:
            format_string = format_string.replace("%M", str(self.minute).zfill(2))
        if "%S" in format_string:
            format_string = format_string.replace("%S", str(self.second).zfill(2))
        if "%F" in format_string:
            format_string = format_string.replace("%F", str(self.millisecond).zfill(2))
        if "%h" in format_string:
            format_string = format_string.replace("%h", str(int(self.hour)).zfill(2))
        if "%m" in format_string:
            format_string = format_string.replace("%m", str(int(self.minute)).zfill(2))
        if "%s" in format_string:
            format_string = format_string.replace("%s", str(int(self.second)).zfill(2))
        if "%f" in format_string:
            format_string = format_string.replace(
                "%f", str(int(self.millisecond)).zfill(2)
            )
    ...
    def to_hour(self):
        return (
            ...
            + self.minute / 60
            + self.second / 3600
            + self.millisecond / 3600000
        )

    def to_minute(self):
        return (
            self.hour * 60 + self.minute + self.second / 60 + self.millisecond / 60000
        )

    def to_second(self):
        return (
            self.hour * 3600 + self.minute * 60 + self.second + self.millisecond / 1000
        )
...
class Video:
    ...
    def set_size(self, size):
        ...
        self._aspect_surf = pygame.transform.scale(
            self._aspect_surf, (self.frame_width, self.frame_height)
        )
        ...
        if self.keep_aspect_ratio:
            if self.frame_width < self.frame_height:
                ...
                frame_surf = pygame.transform.scale(
                    self.frame_surf,
                    (self.frame_width, int(self.frame_width / self.aspect_ratio)),
                )
                self._aspect_surf.blit(
                    frame_surf, (0, self.frame_height / 2 - frame_surf.get_height() / 2)
                )
                ...
                frame_surf = pygame.transform.scale(
                    self.frame_surf,
                    (int(self.frame_height / self.aspect_ratio2), self.frame_height),
                )
                self._aspect_surf.blit(
                    frame_surf, (self.frame_width / 2 - frame_surf.get_width() / 2, 0)
                )
                return self._aspect_surf
        else:
            return pygame.transform.scale(
                self.frame_surf, (self.frame_width, self.frame_height)
            )
